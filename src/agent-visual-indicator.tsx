 ...
 *
 * The scanner bar and stop badge are Preact components rendered into isolated
 * Shadow-DOM hosts, so the host page's CSS can never distort the frame, type, or
 * logo. The opacity envelope — a fast attack, a minimum hold at full brightness,
 * then a gentle capacitor-style release — lives in the pure, unit-tested
 * `indicator-envelope` state machine. This file is the DOM/Chrome adapter: it
 * supplies the visual effects (opacity transitions on the hosts, element
 * teardown) and the real clock + timers, and wires runtime messages to the
 * envelope. The service worker fires show/hide around every tool call, so the
 * hold plus the slow release bridge the brief gaps between consecutive calls: a
 * burst reads as one steady, breathing shimmer.
 *
...
const RED = COLORS.f5Red; // xcsh CLI frame red — keeps brand parity with the terminal UI
...
// Red "scanner" bar: a wide, bright highlight glides back and forth over a tinted
// track, with a soft glow bloom beneath it. The sweep uses ease-in-out so it slows
// and reverses smoothly at each edge (velocity hits zero at the turn) — a clear but
// non-flickering "agent is thinking" signal. overflow:hidden on the host clips the
// highlight to the bar height; the host's own box-shadow is the part that blooms
// below.
function ScannerBar() {
  const css =
    ':host{display:block}' +
...
// frame. The whole emblem is a single click target that stops the agent — there is
// no visible button; clicking anywhere on it sends `stop_agent`. The host stays
// pointer-events:none while faded out (see the envelope effects) so the invisible
...
function createElements(): void {
  ...
  render(<ScannerBar />, scanner.attachShadow({ mode: 'open' }));
  ...
  render(<StopBadge />, badge.attachShadow({ mode: 'open' }));
