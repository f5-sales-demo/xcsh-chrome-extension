...
"Injects the accessibility-tree content script into F5 XC console pages to serialize the page's DOM structure (element roles, names, and coordinates) for deterministic element resolution."
...
"Reads enterprise-managed Chrome policy (managed_schema.json) for IT-configurable domain allowlists and blocked URL patterns. Extension storage holds the selected loopback bridge port and bounded service-worker timing/state metrics; tenant, session, URL, page-content, chat, and credential values are excluded."
