...
describe('planSkillsRequest (session-level, so no turn id to carry)', () => {
  ...
  test('routes to the tab’s own worker under the SAME tenant rules as a turn', () => {
    expect(planSkillsRequest({ tabId: 1, sessionKey: 'acme|staging' }, reg, allOpen)).toEqual({
      kind: 'route',
      port: 19222,
    });
  });
  ...
  test('skips when the socket is closed', () => {
    expect(planSkillsRequest({ tabId: 1, sessionKey: 'acme|staging' }, reg, () => false)).toEqual({ kind: 'skip' });
  });
});
