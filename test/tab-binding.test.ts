...
describe('sessionKeyFromUrl', () => {
  ...
  it('keys a staging tenant console by (tenant, staging)', () => {
    expect(sessionKeyFromUrl('https://example-id-006.staging.volterra.us/web/home')).toEqual({
      tenant: 'example-id-006',
      env: 'staging',
    });
  });
...
describe('sessionKeyStr', () => {
  ...
  it('round-trips from a URL', () => {
    ...
    expect(key && sessionKeyStr(key)).toBe('example-id-006|staging');
  });
});
