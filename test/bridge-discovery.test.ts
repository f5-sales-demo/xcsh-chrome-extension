...
describe('bridge-discovery', () => {
  ...
  test('tenantToPort maps only bridges that reported tenant+env', () => {
    const m = reg([19222, 'alpha', 'staging', 0], [19223, null, null, 0], [19224, 'example-id-009', 'production', 0]);
    const t = tenantToPort(m);
    expect(t.get('example-id-004')).toBe(19222);
    expect(t.get('example-id-003')).toBe(19224);
    expect(t.size).toBe(2);
  });

  test('portForTenant resolves a session key, undefined when absent/null', () => {
    const m = reg([19222, 'alpha', 'staging', 0]);
    expect(portForTenant(m, 'example-id-004')).toBe(19222);
    expect(portForTenant(m, 'example-id-009|staging')).toBeUndefined();
  ...
  test('liveTenants lists session keys with a tenant+env and their contextBound state', () => {
    const m = reg([19222, 'alpha', 'staging', 0, true], [19223, null, null, 0], [19224, 'example-id-009', 'production', 0]);
  ...
  test('liveTenants + tenantToPort exclude asymmetric frames (tenant XOR env)', () => {
    ...
    expect(liveTenants(m)).toEqual([{ tenant: 'example-id-002', contextBound: false }]);
    expect([...tenantToPort(m).keys()]).toEqual(['example-id-002']);
    expect(portForTenant(m, 'tenantonly|production')).toBeUndefined();
    expect(portForTenant(m, 'example-id-002')).toBe(19222);
  });
});
